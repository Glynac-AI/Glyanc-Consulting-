import Navigation from "../../components/Navigation";
import Footer from "../../components/Footer";

const Leadership = () => {
  return (
    <div className="min-h-screen bg-white">
      <Navigation />
      
      {/* Hero Section */}
      <section className="py-20 bg-lcw-primary text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-5xl lg:text-6xl font-display font-bold mb-8">
              Inclusive Leader Development
            </h1>
            <p className="text-xl text-lcw-light-gray mb-10 max-w-3xl mx-auto leading-relaxed font-body">
              Develop your managers and leaders with programs designed to build inclusive leadership capabilities.
            </p>
          </div>
        </div>
      </section>

      {/* Content Section */}
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-4xl lg:text-5xl font-display font-bold text-lcw-text-dark mb-6">
              Building inclusive leadership
            </h2>
            <p className="text-xl text-lcw-text-light font-body max-w-3xl mx-auto leading-relaxed">
              Our programs help leaders develop the skills and mindsets needed to create inclusive, high-performing teams.
            </p>
          </div>

          <div className="grid md:grid-cols-2 gap-12">
            <div>
              <h3 className="text-2xl font-display font-bold text-lcw-text-dark mb-4">
                Leadership Development Programs
              </h3>
              <p className="text-lcw-text-light font-body leading-relaxed mb-6">
                Comprehensive programs designed to build inclusive leadership capabilities at all levels.
              </p>
              <ul className="space-y-3 text-lcw-text-light font-body">
                <li>• Executive leadership development</li>
                <li>• Middle management training</li>
                <li>• Team leader workshops</li>
                <li>• Individual coaching sessions</li>
              </ul>
            </div>
            <div>
              <h3 className="text-2xl font-display font-bold text-lcw-text-dark mb-4">
                Cultural Competence Skills
              </h3>
              <p className="text-lcw-text-light font-body leading-relaxed mb-6">
                Essential skills for leading diverse teams and creating inclusive workplace cultures.
              </p>
              <ul className="space-y-3 text-lcw-text-light font-body">
                <li>• Cross-cultural communication</li>
                <li>• Unconscious bias awareness</li>
                <li>• Inclusive decision-making</li>
                <li>• Conflict resolution</li>
              </ul>
            </div>
          </div>
        </div>
      </section>

      <Footer />
    </div>
  );
};

export default Leadership; 